...
func TestUninstallLeavesSomebodyElsesComposeFileAlone(t *testing.T) {
	...
	// A picture is recognised by its bytes: this one is theirs, so it stays.
	theirPicture := filepath.Join(dir, wizard.SampleName)
	if err := os.WriteFile(theirPicture, []byte("\x89PNG not ours"), 0o600); err != nil {
		t.Fatal(err)
	}
	ourPicture := filepath.Join(dir, "keep", wizard.SampleName)
	if err := os.MkdirAll(filepath.Dir(ourPicture), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(ourPicture, []byte(wizard.SampleImage()), 0o600); err != nil {
		t.Fatal(err)
	}
	if !writtenByGoDrop(ourPicture) {
		t.Error("the picture setup draws should be recognised as ours")
	}
	...
	if _, err := os.Stat(theirPicture); err != nil {
		t.Errorf("a picture GoDrop did not draw must survive: %v", err)
	}
